...
- serializing DFA and NFA models into JSON

- reconstructing executable automata from serialized JSON
...
- AutomataSerializer
  ...
  - Converts DFA and NFA instances into portable JSON

  - Validates serialized data before reconstructing executable automata
...
Automata Builder supports converting DFA and NFA instances into portable JSON and reconstructing executable automata from that JSON.

Serialization is handled by `AutomataSerializer`, rather than by methods inside `Automata`, `DFA`, or `NFA`. This keeps the core automata classes focused on automata behavior and separates persistence concerns from the mathematical model.
...
- all states, including their names and acceptance status

- all transitions, including epsilon transitions represented by `null`
...
An epsilon transition in an NFA is represented with a `null` symbol:
...
An automaton can be serialized into a JSON string:

```ts
const json = AutomataSerializer.serialize(automata);
```

It can later be reconstructed as an executable `DFA` or `NFA`:

```ts
const automata = AutomataSerializer.deserialize(json);
```

The deserializer treats parsed JSON as untrusted data. Before constructing the automaton, `validateSerializedAutomata()` verifies the structure, state and transition identifiers, transition endpoints, and rules specific to each automaton type.

The serialized format intentionally contains only the automata model. UI information such as state coordinates, zoom level, selected elements, and canvas position is not part of the core automata serialization.
...
- DFA and NFA serialization: implemented
